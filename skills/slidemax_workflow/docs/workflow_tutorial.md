...
本教程将通过实际案例，手把手教你如何使用 SlideMax 系统创建专业的 SVG 演示文稿。
...
│  阶段 0: 源内容转换（强制触发）                                               │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │   PDF   → python3 skills/slidemax_workflow/commands/pdf_to_md.py <文件路径>                       │   │
│  │   URL   → python3 skills/slidemax_workflow/commands/web_to_md.py <URL>                           │   │
│  │          （微信/高防站点用 node skills/slidemax_workflow/commands/web_to_md.cjs <URL>）            │   │
│  │   MD    → 直接读取                                                    │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘
                                     │
...
7. 图片使用：
   ...
   ⚠️ 若选择包含「B) 用户提供」，八项确认后需运行 `python3 skills/slidemax_workflow/commands/analyze_images.py <项目路径>/images`，
