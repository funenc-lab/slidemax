...
本文档提供了使用 SlideMax 系统时的详细设计指南和最佳实践。系统支持多种画布格式，包括演示文稿、社交媒体、营销物料等。
...
```
H1 - 封面标题:     60-72px, Bold     (2.5-3×)
H2 - 页面标题:     42-48px, Bold     (1.75-2×)
H3 - 章节标题:     32-36px, SemiBold (1.3-1.5×)
正文 - 主要内容:   24px, Regular     (1× 基准)
小字 - 注释说明:   18-20px, Regular  (0.75-0.85×)
```
...
```
H1 - 封面标题:     45-54px, Bold     (2.5-3×)
H2 - 页面标题:     27-36px, Bold     (1.5-2×)
H3 - 章节标题:     22-27px, SemiBold (1.2-1.5×)
正文 - 主要内容:   18px, Regular     (1× 基准)
小字 - 注释说明:   14-15px, Regular  (0.75-0.85×)
```
...
- 发布/后处理：推荐使用统一后处理工具 `skills/slidemax_workflow/commands/finalize_svg.py`，已包含文本扁平化功能：
  ```bash
