...
1. 新增 `skills/slidemax_workflow/` 作为 workflow-facing skill 容器
2. 将用户可执行脚本、角色协议与工作流文档收口到 skill 内
3. 将共享 Python 核心迁移到 `skills/slidemax_workflow/slidemax/`
...
2. 为 `slidemax/` 增加自动化测试
3. 将导出器、校验器、项目模型进一步模块化

## 增量进展（继续整理）

1. 将 `svg_to_pptx.py` 的导出主循环进一步下沉到 `slidemax.exporters.pptx_runtime`
2. 将 CLI 入口收敛为依赖检查 + 请求组装 + 共享服务调用
3. 修复 package 结构下继续演进时的扩展锚点，使导出运行时成为明确边界

## 增量进展（继续整理：finalize）

1. 新增 `slidemax.finalize_steps`，将后处理步骤定义与 handler 从 `finalize.py` 中拆出
...
1. 新增 `slidemax.svg_processing.icons` 与 `slidemax.svg_processing.embed_images`
...
1. 新增 `slidemax.svg_processing.crop_images` 与 `slidemax.svg_processing.image_aspect`
...
1. 新增 `slidemax.svg_processing.image_utils`，统一 Pillow 检测、图片尺寸读取、MIME 推断、SVG 资源路径解析
2. `embed_images`、`crop_images`、`fix_image_aspect` 已改为复用共享图片工具，减少重复实现
3. 共享图片能力形成独立扩展边界，后续新增图片型步骤可直接复用

## 增量进展（继续整理：core-packaging)

1. `slidemax.config`、`slidemax.project_utils`、`slidemax.error_helper` 已成为真实共享实现
2. `commands/config.py`、`commands/project_utils.py`、`commands/error_helper.py` 已收敛为 thin command wrappers
3. 已移除 `slidemax/_legacy.py` 与对应动态导入链路

## 增量进展（继续整理：pptx-animations）

1. `slidemax.pptx_animations` 已成为动画与切换 XML 的真实共享实现
2. `commands/pptx_animations.py` 已收敛为兼容桥接层
3. `svg_to_pptx.py` 已直接依赖 package 内动画模块，导出链路进一步收口到 `slidemax/`

## 增量进展（继续整理：svg-positioning）

1. `slidemax.svg_positioning` 已成为图表坐标计算与 SVG 坐标校验的真实共享实现
...
1. 新增 `skills/slidemax_workflow/SKILL.md` 作为仓库内 skill 入口
...
4. 将共享核心迁移到 `skills/slidemax_workflow/slidemax/`，并收敛为 skill 内唯一共享核心
...
1. `slidemax.rounded_rect_cli` 与 `slidemax.flatten_text_cli` 已成为简单 SVG 后处理命令的共享 CLI 编排层
...
1. `slidemax.svg_asset_cli` 已成为图片裁剪、图标嵌入、图片嵌入与宽高比修复命令的共享 CLI 编排层
...
1. `slidemax.image_generation` 已吸收 Gemini 兼容入口与 provider smoke test CLI 编排
...
1. `slidemax.image_analysis` 已补齐标准 parser/main 入口
2. `commands/analyze_images.py` 已收敛为薄桥接层
...
1. 移除重复共享模块 `slidemax.rounded_rects_cli`，统一保留 `slidemax.rounded_rect_cli` 作为 canonical 名称
2. `slidemax.__init__` 与架构文档已同步去除旧导出与旧命名
3. 圆角矩形 CLI 的扩展边界统一收口到 `rounded_rect_cli`

## 增量进展（继续整理：dedupe-svg-postprocess-cli）

1. 移除未被命令入口使用的重复共享模块 `slidemax.svg_postprocess_cli`
2. 统一由 `slidemax.flatten_text_cli`、`slidemax.rounded_rect_cli` 与 `slidemax.svg_asset_cli` 承担对应后处理命令编排
