...
1.  **运行扫描工具**：使用 bash 运行 `python3 skills/slidemax_workflow/commands/analyze_images.py <项目路径>/images`。
...
> 💡 **重要**: 项目初始化命令 `python3 skills/slidemax_workflow/commands/project_manager.py init` **不再生成** 默认模板。
>
> **你必须执行以下步骤**:
>
...
>
> ```bash
> python3 skills/slidemax_workflow/commands/project_manager.py init <项目名称> --format <画布格式>
> ```
