...
- Recommended command: `skills/slidemax_workflow/commands/image_generate.py`
- The command supports provider selection through `--provider` or `SLIDEMAX_IMAGE_PROVIDER`
- Default output path can be configured through `SLIDEMAX_IMAGE_OUTPUT_DIR`
- Recommended provider presets: `gemini`, `openai-compatible`, `doubao`
- For ARK image-to-video tasks, use `skills/slidemax_workflow/commands/doubao_i2v_task.py`
- Example model override: `doubao-seedream-4.5`
- `skills/slidemax_workflow/commands/nano_banana_gen.py` remains available as a Gemini-only compatibility wrapper
- 命令格式:
  ```bash
  python3 skills/slidemax_workflow/commands/image_generate.py "your prompt" --provider gemini --aspect_ratio 16:9 --image_size 4K --output 项目/images --filename cover_bg
  ```
- Doubao example:
  ```bash
  python3 skills/slidemax_workflow/commands/image_generate.py "your prompt" --provider doubao --model doubao-seedream-4.5 --aspect-ratio 16:9 --image-size 2K --output 项目/images --filename cover_bg
  ```
...
- ⚠️ **水印处理**: Gemini 生成的图片右下角有星星水印，使用以下工具去除：
  - 本项目工具: `python3 skills/slidemax_workflow/commands/gemini_watermark_remover.py <图片路径>`
...
- 工作流要求：
  1. 优先使用 `python3 skills/slidemax_workflow/commands/download_stock_image.py <项目路径> --provider <provider> --source-url <素材页URL> --download-url <图片直链> --filename <目标文件名>` 直接下载并登记
  2. 或使用 `python3 skills/slidemax_workflow/commands/register_stock_image.py <项目路径> --provider <provider> --source-url <素材页URL> --local-file <本地文件路径> --filename <目标文件名>` 登记已下载文件
