"""Shared image generation service for SlideMax."""
...
PROVIDER_RUNTIME_METADATA = {
    "gemini": {
        ...
        "api_key_envs": ("GEMINI_API_KEY", "SLIDEMAX_IMAGE_API_KEY"),
        "base_url_envs": ("GEMINI_BASE_URL", "SLIDEMAX_IMAGE_BASE_URL"),
        "model_envs": ("GEMINI_IMAGE_MODEL", "SLIDEMAX_IMAGE_MODEL"),
    },
    "openai-compatible": {
        "sdk_module": "openai",
        "install_command": "pip install openai",
        "api_key_envs": ("OPENAI_IMAGE_API_KEY", "OPENAI_API_KEY", "SLIDEMAX_IMAGE_API_KEY"),
        "base_url_envs": ("OPENAI_IMAGE_BASE_URL", "OPENAI_BASE_URL", "SLIDEMAX_IMAGE_BASE_URL"),
        "model_envs": ("OPENAI_IMAGE_MODEL", "OPENAI_MODEL", "SLIDEMAX_IMAGE_MODEL"),
    },
    "doubao": {
        "sdk_module": "volcenginesdkarkruntime",
        "install_command": "pip install \"volcengine-python-sdk[ark]\"",
        "api_key_envs": ("ARK_API_KEY", "DOUBAO_API_KEY", "SLIDEMAX_IMAGE_API_KEY"),
        "base_url_envs": ("ARK_BASE_URL", "DOUBAO_BASE_URL", "SLIDEMAX_IMAGE_BASE_URL"),
        "model_envs": ("DOUBAO_IMAGE_MODEL", "SLIDEMAX_IMAGE_MODEL"),
    },
}
...
def _resolve_output_dir(cli_output_dir: Optional[str | Path]) -> Optional[Path]:
    ...
    env_output = _first_env("SLIDEMAX_IMAGE_OUTPUT_DIR")
...
def resolve_provider_config(
    ...
    normalized_provider = normalize_provider(provider or _first_env("SLIDEMAX_IMAGE_PROVIDER"))
    resolved_output_dir = _resolve_output_dir(output_dir)
    resolved_timeout = timeout_seconds or int(_first_env("SLIDEMAX_IMAGE_TIMEOUT") or DEFAULT_TIMEOUT_SECONDS)
    ...
    if normalized_provider == "gemini":
        provider_model = model or _first_env("SLIDEMAX_IMAGE_MODEL", "GEMINI_IMAGE_MODEL") or DEFAULT_MODELS[normalized_provider]
        provider_api_key = api_key or _first_env("SLIDEMAX_IMAGE_API_KEY", "GEMINI_API_KEY")
        provider_base_url = base_url or _first_env("SLIDEMAX_IMAGE_BASE_URL", "GEMINI_BASE_URL")
        provider_endpoint = endpoint or _first_env("SLIDEMAX_IMAGE_ENDPOINT")
        if not provider_api_key:
            raise ValueError(_missing_config_message(normalized_provider, field_name="API key", env_names=("GEMINI_API_KEY", "SLIDEMAX_IMAGE_API_KEY")))
        ...
        if not provider_api_key:
            raise ValueError(_missing_config_message(normalized_provider, field_name="API key", env_names=("OPENAI_IMAGE_API_KEY", "OPENAI_API_KEY", "SLIDEMAX_IMAGE_API_KEY")))
        if not provider_endpoint and not provider_base_url:
            raise ValueError(_missing_config_message(normalized_provider, field_name="base URL", env_names=("OPENAI_IMAGE_BASE_URL", "OPENAI_BASE_URL", "SLIDEMAX_IMAGE_BASE_URL")))
    else:
        provider_model = model or _first_env("SLIDEMAX_IMAGE_MODEL", "DOUBAO_IMAGE_MODEL") or DEFAULT_MODELS[normalized_provider]
        provider_api_key = api_key or _first_env("SLIDEMAX_IMAGE_API_KEY", "DOUBAO_API_KEY", "ARK_API_KEY")
        provider_base_url = base_url or _first_env("SLIDEMAX_IMAGE_BASE_URL", "DOUBAO_BASE_URL", "ARK_BASE_URL")
        provider_endpoint = endpoint or _first_env("SLIDEMAX_IMAGE_ENDPOINT", "DOUBAO_IMAGE_ENDPOINT")
        if not provider_api_key:
            raise ValueError(_missing_config_message(normalized_provider, field_name="API key", env_names=("ARK_API_KEY", "DOUBAO_API_KEY", "SLIDEMAX_IMAGE_API_KEY")))
        if not provider_endpoint and not provider_base_url:
            raise ValueError(_missing_config_message(normalized_provider, field_name="base URL", env_names=("ARK_BASE_URL", "DOUBAO_BASE_URL", "SLIDEMAX_IMAGE_BASE_URL")))
...
def build_parser(
    *,
    ...
    parser.add_argument("--output", "-o", default=None, help="Output directory. Overrides SLIDEMAX_IMAGE_OUTPUT_DIR.")
