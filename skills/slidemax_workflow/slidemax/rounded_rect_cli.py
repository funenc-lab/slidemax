...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='SlideMax - SVG rounded-rect to path tool',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
    %(prog)s skills/slidemax_workflow/examples/demo_project_intro_ppt169_20251211
    %(prog)s skills/slidemax_workflow/examples/demo_project_intro_ppt169_20251211 -s final
    %(prog)s skills/slidemax_workflow/examples/demo_project_intro_ppt169_20251211/svg_output/slide_01_cover.svg
...
def run_cli(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not quiet:
        print('SlideMax - SVG rounded-rect to path tool')
        print('=' * 50)
