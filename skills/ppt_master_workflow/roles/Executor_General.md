...
Use this section before entering detailed SVG production rules.

### Stage Entry Flow

```text
Entering Executor stage
  -> Read the full strategy output
  -> Confirm template status and applicable template files
  -> Confirm all required images are local or intentionally placeholders
  -> Confirm the target canvas format and page plan
  -> Generate slide SVG into `svg_output/`
  -> Generate speaker notes into `notes/total.md`
  -> Hand off to finalize and export
```

### Delivery Contract

The Executor stage should leave the project with:

- A complete slide set in `svg_output/`
- A complete notes document in `notes/total.md`
- Slide layouts that match the agreed format, style, and asset plan
- No unresolved external asset dependencies for delivery slides

### Reliability Rules

- Do not start slide production before strategy output exists.
- Do not ignore template structure when template files are present.
- Do not hotlink remote assets in delivery SVG files.
- Do not finish slide SVG without also producing the notes output.
- Do not send raw `svg_output/` directly for delivery without finalize and export.

### Failure Recovery

- If a required image is missing, route back to `Image_Generator` instead of improvising.
- If a template mismatch is found mid-stage, pause and realign the current page with the correct template mapping.
- If layout quality degrades across pages, finish the draft set first and then hand off to optimization or post-processing.
