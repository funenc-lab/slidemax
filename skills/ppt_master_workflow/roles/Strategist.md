...
Use this section before reading the detailed strategy rules below.
...
Entering Strategist stage
  ...
  -> Write the project design-outline markdown file
...
- One project-local design-outline markdown file
...
- Do not route to Executor until the design-outline markdown file is actually written.

### Failure Recovery

- If template files appear after strategy started, review and update the strategy output before handing off.
- If user images were skipped, run image analysis before finalizing the strategy output.
- If the style choice is unclear, pause and resolve it here instead of pushing uncertainty downstream.
...
> 💡 以下为示例，完整配色方案（14 个行业）请参考 [commands/config.py](../commands/config.py) 中的 `INDUSTRY_COLORS`
...
> 
> 1. **查阅图标索引**：读取 `templates/icons/icons_index.json`
...
**选择 B/C/D/E 时**，在规范中添加「图片资源清单」，每张图片标注状态与来源：

```markdown
## 图片资源清单

| 文件名       | 尺寸     | 比例 | 布局建议                 | 用途     | 类型     | 状态   | 来源                | 生成描述 / 来源备注              |
| ------------ | -------- | ---- | ------------------------ | -------- | -------- | ------ | ------------------- | -------------------------------- |
| cover_bg.png | 1280×720 | 1.78 | 宽幅横图 (适合全屏/插图) | 封面背景 | 背景图   | 待生成 | AI 生成             | 现代科技感抽象背景，深蓝渐变     |
| product.png  | 600×400  | 1.50 | 标准横图 (适合全屏/插图) | 第3页    | 实景照片 | 已有   | 用户提供            | -                                |
| office.jpg   | 1600×900 | 1.78 | 宽幅横图 (适合全屏/插图) | 第4页    | 实景照片 | 已有   | 商用图库(Unsplash)  | 来源页与许可证登记到 manifest 中 |
| team.png     | 400×600  | 0.67 | 标准竖图 (适合左右分栏)  | 第5页    | 插画配图 | 占位符 | 占位符              | 团队协作场景（后期补充）         |
```

**状态说明**：

- **待生成** - 需要 AI 生成，提供详细描述
- **已有** - 用户已有图片或已本地化的商用图库图片，直接放入 `images/` 或 `images/stock/`（比例/布局建议由 `analyze_images.py` 自动扫描填充）
- **占位符** - 暂不处理，SVG 中用虚线框占位

**来源说明**：

- **AI 生成** - 后续进入 Image_Generator 阶段生成
- **用户提供** - 用户直接提供本地素材
- **商用图库(Provider)** - 从可商用图库下载后，使用 `register_stock_image.py` 登记到 `images/stock/manifest.json`
...
> ⚠️ **商用图库衔接**: 如果用户选择 **E) 商用图库**，需在进入 Executor 前先将图片下载到本地，并使用 `register_stock_image.py` 将来源、许可证和本地路径登记到 `images/stock/manifest.json`。
...
1.  **运行扫描工具**：使用 bash 运行 `python3 skills/ppt_master_workflow/commands/analyze_images.py <项目路径>/images`。
...
| **七、图片资源清单**   | 文件名、尺寸、比例、用途、状态、来源、生成描述 / 来源备注（按需） |
...
> 💡 **重要**: 项目初始化命令 `python3 skills/ppt_master_workflow/commands/project_manager.py init` **不再生成** 默认模板。
>
> **你必须执行以下步骤**:
>
> 1. 读取参考模板: `templates/design_spec_reference.md`
> 2. 基于模板结构，结合你的分析内容，**从头生成** 完整的《设计规范与内容大纲》
> 3. 将生成的内容保存到项目文件夹: `workspace/<项目名>.../设计规范与内容大纲.md`
...
>
> ```bash
> python3 skills/ppt_master_workflow/commands/project_manager.py init <项目名称> --format <画布格式>
> ```

策略师的职责是将《设计规范与内容大纲》**保存到已创建的项目文件夹**：

```
workspace/<项目名>_<格式>_<YYYYMMDD>/设计规范与内容大纲.md
```
