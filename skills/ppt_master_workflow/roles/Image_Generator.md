...
Use this section before reading the detailed prompting and generation guidance below.
...
Entering Image_Generator stage
  ...
  -> Classify each image as local, AI-generated, stock, or placeholder
...
|------------|----------------|-------------------|
| AI image | Generate into `images/` | `image_generate.py`, `smoke_test_image_provider.py` |
| Commercial stock image | Normalize into `images/stock/` | `download_stock_image.py`, `register_stock_image.py` |
| User-provided image | Keep on project-local path | `analyze_images.py` when inspection is needed |
...
- If live generation fails, verify credentials and base URL with a smoke test before retrying.
- If a model has provider-specific constraints, record them in workflow docs before treating the setup as production-ready.
- If a stock image is already local, register provenance instead of bypassing the stock manifest flow.

## 核心使命

作为 AI 图片生成专家，接收 Strategist 输出的《设计规范与内容大纲》中的「图片资源清单」，为每张待生成的图片创建优化提示词，并通过 AI 工具生成图片；如来源为商用图库，则负责将本地化素材登记到项目 `images/stock/manifest.json`。

**触发条件**: 需要生成 AI 图片或准备商用图库图片时（独立使用或流程中调用）
...
2. **优化后的图片提示词**（每张图片）
   ...
   - 同时作为图片描述/alt 文本
3. **生成的图片文件**
   - 保存到 `项目/images/` 或 `项目/images/stock/` 目录（取决于来源类型）
   - 按清单中的文件名命名
4. **更新后的图片资源清单**
   - 状态从「待生成」变更为「已生成」
...
**方式一：使用 provider-neutral 命令行工具** ⭐ 推荐

- Recommended command: `skills/ppt_master_workflow/commands/image_generate.py`
- The command supports provider selection through `--provider` or `PPTMASTER_IMAGE_PROVIDER`
- Default output path can be configured through `PPTMASTER_IMAGE_OUTPUT_DIR`
- Recommended provider presets: `gemini`, `openai-compatible`, `doubao`
- For ARK image-to-video tasks, use `skills/ppt_master_workflow/commands/doubao_i2v_task.py`
- Example model override: `doubao-seedream-4.5`
- `skills/ppt_master_workflow/commands/nano_banana_gen.py` remains available as a Gemini-only compatibility wrapper
- 命令格式:
  ```bash
  python3 skills/ppt_master_workflow/commands/image_generate.py "your prompt" --provider gemini --aspect_ratio 16:9 --image_size 4K --output 项目/images --filename cover_bg
  ```
- Doubao example:
  ```bash
  python3 skills/ppt_master_workflow/commands/image_generate.py "your prompt" --provider doubao --model doubao-seedream-4.5 --aspect-ratio 16:9 --image-size 2K --output 项目/images --filename cover_bg
  ```
- Provider reference: `../docs/image_generation_providers.md`
- **生成节奏控制（强制）**：
...
- **完整参数列表**:
  ...
  | `--model` | `-m` | Provider model override | Provider default |

- 支持的宽高比: `1:1`, `2:3`, `3:2`, `3:4`, `4:3`, `4:5`, `5:4`, `9:16`, `16:9`, `21:9`
- 支持的尺寸: `1K`, `2K`, `4K`（默认）
- 使用 `--output` 或 `-o` 参数指定输出目录，图片将保存到与 `image_prompts.md` 相同的 `images/` 目录
- Environment-first configuration is preferred; see `../docs/image_generation_providers.md`
...
- ⚠️ **水印处理**: Gemini 生成的图片右下角有星星水印，使用以下工具去除：
  - 本项目工具: `python3 skills/ppt_master_workflow/commands/gemini_watermark_remover.py <图片路径>`
...
- 适用场景：需要快速获得可商用实景照片、办公场景、背景图
- 推荐站点：Unsplash、Pexels、Pixabay（交付前仍需复核官方条款）
- 工作流要求：
  1. 优先使用 `python3 skills/ppt_master_workflow/commands/download_stock_image.py <项目路径> --provider <provider> --source-url <素材页URL> --download-url <图片直链> --filename <目标文件名>` 直接下载并登记
  2. 或使用 `python3 skills/ppt_master_workflow/commands/register_stock_image.py <项目路径> --provider <provider> --source-url <素材页URL> --local-file <本地文件路径> --filename <目标文件名>` 登记已下载文件
  3. 确认 `images/stock/manifest.json` 已记录来源、许可证和本地路径
  4. 在《设计规范与内容大纲》的图片资源清单中将状态保持为「已有」，来源写为 `商用图库(provider)`

### 4.4 验证阶段

- 确认所有图片已保存到 `images/` 或 `images/stock/` 目录
- 检查文件名与清单一致
- 更新图片资源清单状态为「已生成」
...
3. 生成后将图片重命名为对应文件名
4. 放入 `images/` 目录（商用图库图片放入 `images/stock/`）
```
...
| 方向              | 内容                                                                        |
| ----------------- | --------------------------------------------------------------------------- |
| **交付**          | 所有图片已放入 `项目/images/` 或 `项目/images/stock/` 目录                     |
| **Executor 引用** | `<image href="../images/xxx.png" .../>` 或 `<image href="../images/stock/xxx.jpg" .../>` |
| **路径说明**      | SVG 在 `svg_output/` 目录，普通图片在 `images/`，图库图片在 `images/stock/`，使用相对路径 `../images/` 或 `../images/stock/` |
...
- [ ] 所有图片已保存到 `项目/images/` 或 `项目/images/stock/` 目录
