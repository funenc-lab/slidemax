...
## Operational Executor Quickstart

Use this section before entering detailed top-tier consulting SVG production rules.

### Stage Entry Flow

```text
Entering Top Consultant Executor stage
  -> Read the strategy output and confirm top-tier consulting style is intentional
  -> Confirm template status, key takeaways, and supporting evidence
  -> Confirm all charts, images, and icon decisions are already grounded
  -> Build `svg_output/` with conclusion-first page logic
  -> Build `notes/total.md` with executive-grade speaking flow
  -> Hand off to finalize and export
```

### Delivery Contract

The Top Consultant Executor stage should leave the project with:

- A complete executive-grade slide set in `svg_output/`
- A complete notes document in `notes/total.md`
- Conclusion-first structure with evidence-backed pages
- Delivery-ready, project-local assets only

### Reliability Rules

- Do not use this role unless the project truly requires top-tier consulting delivery quality.
- Do not let visual polish replace logical rigor; page argument quality comes first.
- Do not introduce unsupported evidence, invented benchmarks, or fake precision.
- Do not degrade the conclusion-first structure while chasing visual complexity.
- Do not skip finalize and export after draft completion.

### Failure Recovery

- If a page looks polished but the argument is weak, fix the storyline before polishing visuals.
- If supporting evidence is missing, route back to strategy instead of filling gaps with invented material.
- If the deck loses executive readability, simplify the page architecture before adding more elements.
...
| 状态        | 来源                 | 处理方式                             |
| ----------- | -------------------- | ------------------------------------ |
| **已有**    | 用户提供 / 商用图库  | 直接引用 `../images/` 或 `../images/stock/` 目录中的图片 |
...
完整图表目录和选择指南：[templates/charts/README.md](../templates/charts/README.md)
