...
用法:
    from pptmaster.config import Config, CANVAS_FORMATS, DESIGN_COLORS
...
from .svg_rules import build_svg_constraints
...
EXTRA_EXAMPLE_PATHS_ENV = 'PPTMASTER_EXTRA_EXAMPLE_PATHS'
...
def get_extra_example_dirs(env_value: Optional[str] = None, must_exist: bool = True) -> List[Path]:
    raw_value = os.getenv(EXTRA_EXAMPLE_PATHS_ENV, '') if env_value is None else env_value
    if not raw_value:
        return []
    ...
    extra_dirs = _dedupe_paths(parsed_paths)
    if must_exist:
        extra_dirs = [path for path in extra_dirs if path.is_dir()]

    return extra_dirs


def get_example_dirs(include_default: bool = True, env_value: Optional[str] = None, must_exist: bool = True) -> List[Path]:
    example_dirs: List[Path] = []
    if include_default:
        example_dirs.append(EXAMPLES_DIR)

    example_dirs.extend(get_extra_example_dirs(env_value=env_value, must_exist=False))
    example_dirs = _dedupe_paths(example_dirs)

    if must_exist:
        example_dirs = [path for path in example_dirs if path.is_dir()]
...
SVG_CONSTRAINTS = build_svg_constraints()
...
class Config:
    ...
    @staticmethod
    def get_example_dirs(include_default: bool = True) -> List[Path]:
        """Return example root directories."""
        return get_example_dirs(include_default=include_default)

    @staticmethod
    def get_extra_example_dirs() -> List[Path]:
        """Return extra example directories from the environment."""
        return get_extra_example_dirs()
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("用法:")
        print("  python3 skills/ppt_master_workflow/commands/config.py list-formats     # 列出所有画布格式")
        print("  python3 skills/ppt_master_workflow/commands/config.py list-colors      # 列出所有配色方案")
        print("  python3 skills/ppt_master_workflow/commands/config.py list-industries  # 列出所有行业配色")
        print("  python3 skills/ppt_master_workflow/commands/config.py export           # 导出配置到 JSON")
        print("  python3 skills/ppt_master_workflow/commands/config.py format <key>     # 查看指定画布格式")
        return
