...
from pptmaster.config import EXTRA_EXAMPLE_PATHS_ENV, PROJECT_ROOT, SKILL_ROOT, get_example_dirs
from pptmaster.project_utils import CANVAS_FORMATS, find_all_projects, get_project_info

CANONICAL_COMMANDS_DIR = (PROJECT_ROOT / 'skills' / 'ppt_master_workflow' / 'commands').resolve()
SKILL_RESOURCES = [
...
def build_examples_index(examples_path: Path, now: Optional[datetime] = None) -> ExamplesIndexResult:
    ...
    content: List[str] = []
    content.append('# PPT Master Example Index\n')
    ...
    content.append('')
    content.append('---\n')
    content.append(f'*Generated on {now.strftime("%Y-%m-%d %H:%M:%S")} by PPT Master*')
