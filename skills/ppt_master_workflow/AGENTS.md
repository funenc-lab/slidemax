...
```bash
python3 skills/ppt_master_workflow/commands/project_manager.py init <项目名称> --format <格式>
```
...
**情况 1：模板在 repo 内**（如 `skills/ppt_master_workflow/templates/layouts/` 下）

检查模板目录内容，按类型分别复制：
```bash
# 复制模板文件（.svg, .md）到 templates/
cp skills/ppt_master_workflow/templates/layouts/<模板名>/*.svg <项目路径>/templates/
cp skills/ppt_master_workflow/templates/layouts/<模板名>/design_spec.md <项目路径>/templates/

# 复制图片资源（.png, .jpg 等）到 images/
# 注意：图片直接在模板目录下，不是在 images/ 子目录
cp skills/ppt_master_workflow/templates/layouts/<模板名>/*.png <项目路径>/images/ 2>/dev/null || true
cp skills/ppt_master_workflow/templates/layouts/<模板名>/*.jpg <项目路径>/images/ 2>/dev/null || true
cp skills/ppt_master_workflow/templates/layouts/<模板名>/*.jpeg <项目路径>/images/ 2>/dev/null || true
```
...
1. **阅读角色定义**：
   ```
   读取文件: skills/ppt_master_workflow/roles/Strategist.md
   ```
...
3. **如果用户提供了图片**，运行图片分析：
   ```bash
   python3 skills/ppt_master_workflow/commands/analyze_images.py <项目路径>/images
   ```

4. **生成《设计规范与内容大纲》**：
   - 参考模板：`skills/ppt_master_workflow/templates/design_spec_reference.md`
...
1. **阅读角色定义**：
   ```
   读取文件: skills/ppt_master_workflow/roles/Image_Generator.md
   ```
...
4. **生成图片**（四种方式）：
   ...
   **方式一：命令行工具** ⭐ 推荐
   ...
   - Provider is selected by `--provider` or `PPTMASTER_IMAGE_PROVIDER`.
   - Default output directory can be set by `PPTMASTER_IMAGE_OUTPUT_DIR`.
   ...
   - `nano_banana_gen.py` remains available as a Gemini-only compatibility wrapper.
     ```bash
     python3 skills/ppt_master_workflow/commands/image_generate.py "现代科技感背景" --provider gemini --aspect_ratio 16:9 --image_size 4K -o <项目路径>/images
     ```
   - Doubao example:
     ```bash
     python3 skills/ppt_master_workflow/commands/image_generate.py "现代科技感背景" --provider doubao --model doubao-seedream-4.5 --aspect_ratio 16:9 --image_size 2K -o <项目路径>/images
     ```
   - Detailed provider configuration: `docs/image_generation_providers.md`
   - Detailed usage: `commands/README.md` and `skills/ppt_master_workflow/roles/Image_Generator.md`
   ...
   **方式五：商用图库**
   ...
   - 先下载图片到本地，再登记到项目 `images/stock/manifest.json`
     ```bash
     python3 skills/ppt_master_workflow/commands/register_stock_image.py <项目路径> --provider unsplash --source-url <素材页URL> --local-file <本地文件路径> --filename office_hero.jpg
     ```
...
1. **阅读角色定义**（根据风格选择）：
   ```
   读取文件: skills/ppt_master_workflow/roles/Executor_General.md        # 通用灵活风格
   读取文件: skills/ppt_master_workflow/roles/Executor_Consultant.md     # 一般咨询风格
   读取文件: skills/ppt_master_workflow/roles/Executor_Consultant_Top.md # 顶级咨询风格
   ```
...
1. **阅读角色定义**：
   ```
   读取文件: skills/ppt_master_workflow/roles/Optimizer_CRAP.md
   ```
...
| 阶段         | 必须阅读的文件                     | 触发条件                                     |
| ------------ | ---------------------------------- | -------------------------------------------- |
| 策略规划     | `skills/ppt_master_workflow/roles/Strategist.md`              | 用户提出新的 PPT/内容生成需求                |

| 图片生成     | `skills/ppt_master_workflow/roles/Image_Generator.md`         | 图片方式包含「C) AI 生成」（如 C、B+C、C+D） |
| 通用风格执行 | `skills/ppt_master_workflow/roles/Executor_General.md`        | 用户选择「A) 通用灵活」设计风格              |
| 咨询风格执行 | `skills/ppt_master_workflow/roles/Executor_Consultant.md`     | 用户选择「B) 一般咨询」设计风格              |
| 顶级咨询执行 | `skills/ppt_master_workflow/roles/Executor_Consultant_Top.md` | 用户选择「C) 顶级咨询」设计风格              |
| 视觉优化     | `skills/ppt_master_workflow/roles/Optimizer_CRAP.md`          | 用户要求优化或主动建议优化                   |
...
📖 **阅读角色定义**: `skills/ppt_master_workflow/roles/[角色文件名].md`
📋 **当前任务**: [简述本阶段要完成的任务]
---
```
...
📖 **阅读角色定义**: `skills/ppt_master_workflow/roles/Image_Generator.md`
...
- [x] 已阅读 `skills/ppt_master_workflow/roles/Image_Generator.md`
- [x] 已创建提示词文档 `images/image_prompts.md`
...
| 源内容 | 工具 | 命令 |
|--------|------|------|
| PDF | `pdf_to_md.py` | `python3 skills/ppt_master_workflow/commands/pdf_to_md.py <文件>` |
| 网页 | `web_to_md.py` | `python3 skills/ppt_master_workflow/commands/web_to_md.py <URL>` |
| 微信/高防 | `web_to_md.cjs` | `node skills/ppt_master_workflow/commands/web_to_md.cjs <URL>` |
...
**若图片方案包含「B) 用户提供」**：八项确认完成后、进入内容分析与大纲编制之前，必须运行 `python3 skills/ppt_master_workflow/commands/analyze_images.py <项目路径>/images`，并在输出《设计规范与内容大纲》前填充图片资源清单。
...
> 📖 **详细代码示例**：参见 `skills/ppt_master_workflow/roles/Executor_*.md` 对应章节
...
python3 skills/ppt_master_workflow/commands/web_to_md.py <URL> 或 node skills/ppt_master_workflow/commands/web_to_md.cjs <URL>

# 初始化项目
python3 skills/ppt_master_workflow/commands/project_manager.py init <名称> --format ppt169

# 验证项目
python3 skills/ppt_master_workflow/commands/project_manager.py validate <路径>

# SVG 质量检查
python3 skills/ppt_master_workflow/commands/svg_quality_checker.py <路径>
...
| 场景                         | 推荐工具        | 命令                                                   |
| ---------------------------- | --------------- | ------------------------------------------------------ |
| **微信公众号/高防站点**      | `web_to_md.cjs` | `node skills/ppt_master_workflow/commands/web_to_md.cjs <URL>` (绕过 TLS 拦截，推荐) |
| **普通文章/新闻网页**        | `web_to_md.py`  | `python3 skills/ppt_master_workflow/commands/web_to_md.py <URL>`                     |
| **图文内容**（游记、攻略等） | `web_to_md.py`  | 同上（自动下载图片到 `_files/`）                       |
| **政府/机构网站**            | `web_to_md.py`  | 同上（支持中文站点元数据提取）                         |
| **批量处理多个 URL**         | `web_to_md.py`  | `python3 skills/ppt_master_workflow/commands/web_to_md.py -f urls.txt`               |
...
1. **切换角色前必须阅读角色定义文件**
   - 使用当前环境可用的文件查看方式读取 `skills/ppt_master_workflow/roles/[角色名].md`
   - 不得在未阅读的情况下直接执行任务
...
- 若图片方案包含「B) 用户提供」，策略师在八项确认后、内容分析前必须运行 `python3 skills/ppt_master_workflow/commands/analyze_images.py <项目路径>/images` 并填充图片资源清单
