...
description: Use when generating PPT, XHS, story, or related canvas outputs with this repository, or when maintaining workflow-facing assets under skills/ppt_master_workflow that depend on the repository-specific stage order, role handoffs, command surface, and project structure rules.
---
...
This skill is the canonical entry point for the PPT Master workflow in this repository.

Use it when the task depends on the repository-specific execution order, role protocol, command entry points, project layout, or workflow-facing assets under `skills/ppt_master_workflow/`.
...
- Modify workflow-facing roles, commands, docs, examples, templates, or shared workflow services under `skills/ppt_master_workflow/`.
...
- The task is unrelated to the PPT Master workflow.
...
- `skills/ppt_master_workflow/` is the only workflow source of truth.
...
- `pptmaster/` contains reusable Python services; command scripts should stay thin.
...
1. `skills/ppt_master_workflow/AGENTS.md`
2. `skills/ppt_master_workflow/workflows/generate-ppt.md` only to confirm the redirect behavior
3. The required role file in `skills/ppt_master_workflow/roles/`
4. The relevant command documentation in `skills/ppt_master_workflow/commands/README.md` when running or changing tooling
5. The shared service implementation in `skills/ppt_master_workflow/pptmaster/` when the task is not command-only
...
11. Put reusable logic in `pptmaster/`; keep `commands/` thin.
12. Do not create duplicate workflow sources outside `skills/ppt_master_workflow/`.
...
Fix:
- Treat `skills/ppt_master_workflow/AGENTS.md` as authoritative.
...
6. Keep shared logic changes in `pptmaster/`.
...
Output: Keep `skills/ppt_master_workflow/` as the only workflow source of truth, reduce duplicate entry docs, and ensure this skill points future agents to the correct file order.
...
- `skills/ppt_master_workflow/AGENTS.md` - Canonical workflow rules.
...
- `skills/ppt_master_workflow/docs/ark_video_generation.md` - ARK image-to-video flow.
