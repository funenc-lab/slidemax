...
| 文件名 | 尺寸 | 用途 | 状态 | 来源 | 生成描述 / 来源备注 |
|--------|------|------|------|------|--------------------|
| cover_bg.png | 1280×720 | 封面背景 | 待生成 | AI生成 | 现代科技感抽象背景，深蓝渐变 |
| product.png | 600×400 | 第3页 | 已有 | 用户提供 | - |
| office.jpg | 1600×900 | 第4页 | 已有 | 商用图库(Unsplash) | 来源与许可证记录在 manifest 中 |
| team.png | 600×400 | 第5页 | 占位符 | 占位符 | 团队协作场景（后期补充） |
```
...
| **已有** | 用户已有图片或已本地化的商用图库图片 | 放入 `images/` 或 `images/stock/`，用 `<image>` 引用 |
...
2. 图片准备（状态：待生成/已有）
   ...
   ├── 已有：用户直接提供
   ├── 商用图库：下载到本地后登记到 images/stock/manifest.json
   └── 放入 项目/images/ 或 项目/images/stock/ 目录

3. Executor 生成 SVG（SVG 在 svg_output/ 目录中）
   ├── 已有/待生成 → <image href="../images/xxx.png" .../>
   ├── 商用图库 → <image href="../images/stock/xxx.jpg" .../>
   └── 占位符 → 虚线框 + 描述文本
...
5. 导出（可选）
   └── python3 skills/ppt_master_workflow/commands/embed_images.py *.svg
```
