...
本文档定义 PPT Master 中第三方商用图库图片的来源登记规则。
...
```bash
python3 skills/ppt_master_workflow/commands/download_stock_image.py <项目路径> \
  --provider pexels \
...
```bash
python3 skills/ppt_master_workflow/commands/register_stock_image.py <项目路径> \
  --provider unsplash \
...
- 配置示例：`../examples/config/pptmaster_stock.env.example`
