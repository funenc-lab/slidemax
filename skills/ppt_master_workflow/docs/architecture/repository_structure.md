...
- `skills/ppt_master_workflow/commands/` 是用户可执行命令的真实位置。
- `skills/ppt_master_workflow/pptmaster/` 是共享 Python 核心的真实位置。
- `skills/ppt_master_workflow/roles/`、`workflows/`、`docs/`、`templates/`、`examples/` 是相应资源的真实位置。
- 样式资产（layouts、charts、icons、design spec references）统一收口在 `skills/ppt_master_workflow/templates/`。
- 根目录只保留最小入口文件与 `workspace/`，真实实现与资源全部收口在 skill 内。

### 2. Package-first imports

- 新代码优先从 `pptmaster.*` 导入共享能力。
- 新增 CLI 时，优先落在 `skills/ppt_master_workflow/commands/`。
- 共享逻辑优先沉到 `skills/ppt_master_workflow/pptmaster/`，CLI 只负责参数解析和编排。

### 3. Skill-only source of truth

- 所有工作流命令、共享核心、角色、文档、模板、示例都只在 `skills/ppt_master_workflow/` 内演进。
- 根目录不再保留脚本、目录别名或包装层。
- 用户工作区与实现资源严格分离。
...
- `skills/ppt_master_workflow/workflows/`
- `skills/ppt_master_workflow/references/`
...
- 内置示例根目录通过 `get_example_dirs()` 统一发现，默认包含 skill 内置 `skills/ppt_master_workflow/examples/`，并可通过环境变量 `PPTMASTER_EXTRA_EXAMPLE_PATHS` 追加外部样例根目录
- `PPTMASTER_EXTRA_EXAMPLE_PATHS` 使用系统路径分隔符（macOS/Linux 为 `:`，Windows 为 `;`）

### 项目领域扩展

- `pptmaster.project_utils`
- 适合新增：项目命名解析、结构校验、元数据索引、批量扫描能力

### 错误与诊断扩展

- `pptmaster.error_helper`
- 适合新增：统一错误码、分级诊断、机器可读错误输出

### 后处理编排扩展

- `pptmaster.finalize`
- `pptmaster.finalize_steps`
- `pptmaster.finalizers.*`
- `pptmaster.svg_processing.*`
- `pptmaster.svg_processing.image_utils`
- `pptmaster.svg_positioning`
...
- `pptmaster.svg_quality`
- 适合新增：规则集合、问题分类、目录扫描、报告格式与批量项目校验

### 批量项目校验扩展

- `pptmaster.batch_validation`
- 适合新增：目录选择策略、项目聚合校验、摘要统计、退出码策略与文本报告导出

### 项目管理扩展

- `pptmaster.project_management`
- 适合新增：项目初始化策略、README 模板、单项目校验编排、信息摘要输出

### 商用图库 CLI 扩展

- 当前命令桥接直接复用 `pptmaster.stock_sources` 中的 CLI helper
- 适合新增：provider 列表输出、参数解析、下载与登记编排、CLI 入口复用

### 视频生成 CLI 扩展

- `pptmaster.video_generation_cli`
- 适合新增：子命令解析、任务状态输出、下载命令编排

### 导出服务扩展

- `pptmaster.pptx_export`
- `pptmaster.pptx_animations`
- 适合新增：导出请求模型、输出命名策略、notes 策略、切换/动画 XML 生成、不同导出后端

### 导出子模块扩展

- `pptmaster.exporters.*`
- 当前推荐入口：`pptmaster.exporters.pptx_runtime.export_presentation`
